...
// getAllCategoriesForDisplay এবং getCategoryName ইম্পোর্ট করা হচ্ছে utils/categories.js থেকে
import { getAllCategoriesForDisplay, getCategoryName } from '../utils/categories'; // সঠিক পাথ নিশ্চিত করুন
import { generateSlug } from '../utils/slugify'; // সঠিক পাথ নিশ্চিত করুন

const ITEMS_PER_PAGE = 10;

const Home = ({ initialJobs, totalJobs, allCategoriesFromSSR }) => {
  const safeInitialJobs = Array.isArray(initialJobs) ? initialJobs : [];
  // allCategoriesFromSSR এখন id, name, slug সহ আসবে
  const safeAllCategories = Array.isArray(allCategoriesFromSSR) ? allCategoriesFromSSR : [];

  const [currentPage, setCurrentPage] = useState(1);
  const [jobs, setJobs] = useState([]);
  const [selectedCategory, setSelectedCategory] = useState(null); // ডিফল্ট null, অর্থাৎ "সকল চাকরি"
  const [searchTerm, setSearchTerm] = useState('');

  const getFilteredJobs = useCallback(() => {
    let filtered = safeInitialJobs;
    if (selectedCategory) { // selectedCategory একটি আইডি (Number) হবে
  ...
  const filterJobsByCategory = (categoryId) => { // categoryId একটি নাম্বার হবে
    setSelectedCategory(categoryId);
    setCurrentPage(1);
    setSearchTerm('');
  };

  // "সকল চাকরি" লিংকে বা লোগোতে ক্লিক করলে এই ফাংশন কল হবে
  ...
  const handleSearch = (event) => {
    ...
    setCurrentPage(1);
    setSelectedCategory(null); // সার্চ করলে "সকল চাকরি" ভিউতে ফিরে যাবে
  };
  ...
  // এই ফাংশনটি আর ব্যবহৃত হচ্ছে না কারণ জব কার্ড থেকে ক্যাটাগরি ট্যাগ বাদ দেওয়া হয়েছে
  // const getJobCategoryNameForCard = (categoryIds) => { ... };

  const noJobsMessage = searchTerm
    ? `"${searchTerm}" এর জন্য কোনো ফলাফল পাওয়া যায়নি।`
    : (selectedCategory
        ? `${getCategoryName(selectedCategory)} ক্যাটাগরিতে কোনো চাকরি নেই।` // getCategoryName আইডি দিয়ে কল করা হবে
  ...
  return (
    <div className="bg-gray-100 min-h-screen flex flex-col">
      ...
      <Header
        allCategories={safeAllCategories} // এখন id, name, slug সহ অবজেক্ট এর অ্যারে
        currentSelectedCategory={selectedCategory} // null অথবা ক্যাটাগরি আইডি (Number)
        onSelectAllJobs={handleSelectAllJobs} // "সকল চাকরি" রিসেট করার ফাংশন
      />
          ...
          <div className="space-y-6">
            {jobs && jobs.length > 0 ? (
              jobs.map((job) => {
                ...
                // const categoryName = getJobCategoryNameForCard(job.category_id); // আপনি এটি বাদ দিয়েছেন
                    ...
                    <div className="p-6 flex flex-col justify-between flex-grow">
                      <div>
                        ...
                        <div className="flex items-center text-xs md:text-sm text-gray-500 mb-3 flex-wrap gap-x-3 gap-y-1">
                          ...
                          {/* ক্যাটাগরি ট্যাগ বাদ দেওয়া হয়েছে আপনার অনুরোধ অনুযায়ী */}
                        </div>
          ...
          <div className="bg-white shadow-lg rounded-lg p-4 sticky top-24 w-full">
            ...
            {(safeAllCategories && safeAllCategories.length > 0) ? (
              <ul className="space-y-1 max-h-96 overflow-y-auto py-1">
                {/* "সকল চাকরি" অপশনটি সাইডবার থেকে বাদ দেওয়া হয়েছে */}
...
export async function getServerSideProps() {
  ...
  let initialJobs = [];
  let totalJobs = 0;
  // getAllCategoriesForDisplay ব্যবহার করে সব ক্যাটাগরি (id, name, slug সহ) লোড করা হচ্ছে
  ...
  try {
    ...
    initialJobs = initialJobs.map(job => ({
        ...job,
        // category_id একটি একক মান বা অ্যারে হতে পারে, সেটাকে Number বা Number অ্যারেতে রূপান্তর করা হচ্ছে
        category_id: Array.isArray(job.category_id)
            ? job.category_id.map(id => Number(id))
            : (job.category_id ? Number(job.category_id) : null)
            // যদি আপনি চান category_id সবসময় একটি অ্যারে হবে, তাহলে নিচের মতো করতে পারেন:
            // category_id: job.category_id
            //     ? (Array.isArray(job.category_id) ? job.category_id.map(Number) : [Number(job.category_id)])
            //     : []
    }));
  ...
  return {
    props: {
      ...
      totalJobs: totalJobs,
      allCategoriesFromSSR: serializableCategories // এখানে id, name, slug সহ অবজেক্ট থাকবে
    }
  };
}
